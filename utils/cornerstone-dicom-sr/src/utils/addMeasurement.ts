...
import { metaData, utilities, Types as csTypes, RenderingEngine } from '@cornerstonejs/core';
...
export default function addMeasurement(
  ...
  // measurementData.renderableData['POLYLINE'].forEach((polyline, index) => {
  //   customAnnotations.data.polyline = polyline;
  //   annotationManager.addAnnotation(customAnnotations);
  // })
  ...
  switch (GraphicType) {
    ...
    case SCOORD_TYPES.POLYLINE:
      ...
      for (let i = 0; i < GraphicData.length; i += 2) {
        ...
        const worldPos = imageToWorldCoords(imageId, [  //utilities.imageToWorldCoords(imageId, [  changed imageToWorldCoords funcion => undefined values
          GraphicData[i],
          GraphicData[i + 1],
        ]);
